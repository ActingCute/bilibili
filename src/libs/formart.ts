...
class FormartData {
  ...
  formartData = (res: Object[]) => {
     ...
     */
    var ret: Object[] = [];
    res.forEach(function (bangumi: any) {
      var temp: any = {};
      ...
      if (bangumi["is_finish"]) {
        ...
        if (!isNaN(total)) total = bangumi["total_count"]; //有些最后是Extra,默认识别为最后一集
      }
      if (total < 0) total = 0;
      temp["all"] = total;
      ...
      if (!bangumi["is_started"]) {
        ...
        if (
          bangumi["progress"].indexOf("第") != -1 &&
          bangumi["progress"].indexOf("话") != -1
        ) {
          var ep_text = bangumi["progress"].match(/第(\S*)话/);
          if (ep_text) if (ep_text.length > 0) ep = ep_text[1];
        }
      } else {
        ep = 0;
      }
